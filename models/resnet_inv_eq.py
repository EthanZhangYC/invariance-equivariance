...
class ResNet_2ndhalf(nn.Module):

    def __init__(self, block, n_blocks, keep_prob=1.0, avg_pool=False, drop_rate=0.0,
                 dropblock_size=5, num_classes=-1, no_trans=16, embd_size=64, use_se=False):
        super(ResNet_2ndhalf, self).__init__()
    ...
    def forward(self, x, is_feat=False, inductive=False):
        ...
        if inductive:
            eq_logit = self.eq_head(conv_feat)
            inv_logit = self.inv_head(conv_feat)
            return pred, conv_feat, eq_logit, inv_logit
...
def resnet12_1st_half(keep_prob=1.0, avg_pool=False, **kwargs):
    """Constructs a ResNet-50 model.
    indeed, only (3 + 4 + 6 + 3) * 3 + 1 = 49 layers
    """
    model = ResNet_1sthalf(BasicBlock, [1, 1, 1, 1], keep_prob=keep_prob, avg_pool=avg_pool, **kwargs)
    return model

def resnet12_2nd_half(keep_prob=1.0, avg_pool=False, **kwargs):
    """Constructs a ResNet-50 model.
    indeed, only (3 + 4 + 6 + 3) * 3 + 1 = 49 layers
    """
    model = ResNet_2ndhalf(BasicBlock, [1, 1, 1, 1], keep_prob=keep_prob, avg_pool=avg_pool, **kwargs)
    return model
